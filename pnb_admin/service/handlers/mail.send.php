...
function send_mail_to_member($desc) {
   ...
   .'This is an automatically generated Email showing below the information we have for you as a sPHENIX user in the sPHENIX PhoneBook.'."\n"
...
//	if ( $to == 'arkhipkin@bnl.gov' || $to == 'morrison@bnl.gov' ) {
...
//	if ( 'dave@bnl.gov' === $to ) {
//		mail_utf8('dave@bnl.gov',$subject,$message,$additional);
//	}
//	mail_utf8($to,$subject,$message,$additional);
...
function mail_send_handler($params) {
  ...
  $mem = file_get_contents('https://www.sphenix.bnl.gov/pnb/service/?q=/members/list/status:active/details:full');
  $mem = json_decode($mem, true);

  $mfields = file_get_contents('https://www.sphenix.bnl.gov/pnb/service/?q=/service/list/object:fields/type:members');
  $mfields = json_decode($mfields, true);

  unset($mfields[6]); // inspire id
  unset($mfields[23]); // cell phone
  ...
  $mfieldgrps = file_get_contents('https://www.sphenix.bnl.gov/pnb/service/?q=/service/list/object:fieldgroups/type:members');
  $mfieldgrps = json_decode($mfieldgrps, true);

  $inst = file_get_contents('https://www.sphenix.bnl.gov/pnb/service/?q=/institutions/list/status:active/details:full');
  $inst = json_decode($inst, true);

  $ifields = file_get_contents('https://www.sphenix.bnl.gov/pnb/service/?q=/service/list/object:fields/type:institutions');
  ...
  foreach( $mem as $k => $v ) {
    ...
    if ( empty($v['fields'][17]) || empty($inst[$inst_id]) || ( !empty($inst[$inst_id]['fields'][42]) && ($inst[$inst_id]['fields'][42] != '0000-00-00 00:00:00') ) ) {
        unset($mem[$k]);
    }
    if (
		  !empty( $v['fields'][85] )

...
function members_excelx_handler($params) {
  ...
  $query = 'SELECT * FROM `'.$db_name.'`.`members` WHERE `status` != "inactive"';
  $mem = $db->Query($query);
  ...
  header('Content-Disposition: attachment;filename="sphenix-phonebook-'.time(0).'.xlsx"');
...
//	$xls->addArray($out);
//	$xls->generateXML('sphenix_collaboration_'.date('Ymd'));

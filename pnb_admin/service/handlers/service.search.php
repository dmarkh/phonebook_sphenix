...
function search_members($field_id, $keyword, $type, $autocomplete) {
	...
	switch ($type) {
		...
		case 'empty':
			...
			$query = 'SELECT m.id as members_id, "" as value FROM `'.$db_name.'`.`members` m WHERE m.status = "active" AND NOT EXISTS( SELECT * FROM `'.$db_name.'`.`members_data_'.$fields[$field_id]['type'].'s` mds WHERE mds.members_fields_id = '.$field_id.' AND mds.members_id = m.id )';
			$res = $db->Query($query);
			break;
		case 'equals':
		case 'default':
...
function search_institutions($field_id, $keyword, $type, $autocomplete) {
	...
	switch ($type) {
		...
		case 'soundex':
			...
			break;
		case 'empty':
			$query = 'SELECT m.id as institutions_id, "" as value FROM `'.$db_name.'`.`institutions` m WHERE m.status = "active" AND NOT EXISTS( SELECT * FROM `'.$db_name.'`.`institutions_data_'.$fields[$field_id]['type'].'s` mds WHERE mds.institutions_fields_id = '.$field_id.' AND mds.institutions_id = m.id )';
			$res = $db->Query($query);
			break;
		case 'equals':
		case 'default':

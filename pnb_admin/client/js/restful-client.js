...
RCLIENT.Phonebook = function () {
	...
	this.tabCounter = 4;
        ...
        this.service_url = 'https://www.sphenix.bnl.gov/pnb_admin/service/index.php';
...
RCLIENT.Phonebook.prototype = {
	...
	get_authorlist_xml: function() {
        ...
        var service = this, field, ifield, inst_id, f_leave_date = this.find_field_id_members('date_leave'),
			...
			f_mem_first_name = this.find_field_id_members('name_first'),
            f_mem_initials = this.find_field_id_members('name_initials'),
            f_mem_orcid = this.find_field_id_members('orcid_id'),
            f_mem_inspire_id = this.find_field_id_members('inspire_id'),
			f_mem_institution_id = this.find_field_id_members('institution_id'),
			f_mem_extra_institution_id = this.find_field_id_members('extra_institution_id'),
            f_inst_id = this.find_field_id_members('institution_id'),
            f_inst_full_name = this.find_field_id_institutions('name_full'),
            f_inst_short_name = this.find_field_id_institutions('name_short'),
            f_inst_latex_name = this.find_field_id_institutions('name_latex'),
			f_inst_inspire_id = this.find_field_id_institutions('INSPIRE_ID'),
			f_inst_address_line_1 = this.find_field_id_institutions('address_line_1'),
			f_inst_address_line_2 = this.find_field_id_institutions('address_line_2'),
        ...
        for ( var i in this.members ) {
			...
			// list of institutions for XML processing
            if ( !affiliations[ inst_id ] ) {
                ifield = this.institutions[ inst_id ]['fields'];
                inst_latex_name = ifield[ f_inst_latex_name ];
				affiliation = {
				  id: inst_id,
				  latex_name: ifield[ f_inst_latex_name ],
				  full_name: ifield[ f_inst_full_name ],
				  short_name: ifield[ f_inst_short_name ],
				  inspire_id: ifield[ f_inst_inspire_id ],
				  address_line_1: ifield[ f_inst_address_line_1 ],
				  address_line_2: ifield[ f_inst_address_line_2 ],
				  city: ifield[ f_inst_city ],
				  country: ifield[ f_inst_country ],
				  postcode: ifield[ f_inst_postcode ],
				};
				let address = [];
				if ( affiliation.address_line_1 ) { address.push( affiliation.address_line_1 ); }
				if ( affiliation.address_line_2 ) { address.push( affiliation.address_line_2 ); }
				if ( affiliation.city ) { address.push( affiliation.city ); }
				if ( affiliation.country ) { address.push( affiliation.country ); }
				if ( affiliation.postcode ) { address.push( affiliation.postcode ); }
				affiliation.address = address.join(', ');

                affiliations[ inst_id ] = affiliation;
                inst_list.push( affiliation );
            }
            // full member name
            if ( field[ f_mem_initials ] && field[ f_mem_initials ].length > 0 ) {
              member_full = this.make_nonbreaking_space( field[ f_mem_initials ] ) + '~' +
                          ( ( field[ f_mem_latex_last_name ] && field[ f_mem_latex_last_name ].length > 1 ) ?
                          field[ f_mem_latex_last_name ] : field[ f_mem_last_name ] );
            } else {
              member_full = field[ f_mem_first_name ].charAt(0).toUpperCase() + '.~' +
                          ( ( field[ f_mem_latex_last_name ] && field[ f_mem_latex_last_name ].length > 1 ) ?
                          field[ f_mem_latex_last_name ] : field[ f_mem_last_name ] );
            }
			// member information
			  ...
			  'initials': field[ f_mem_initials ] || '',
			  'lastname': field[ f_mem_last_name ] || '',
			  'firstname': field[ f_mem_first_name ] || '',
			  'papername': member_full || '',
			  'institutions': [ field[ f_mem_institution_id ] ],
			  'orcid': field[ f_mem_orcid ],
			  'inspire_id': field[ f_mem_inspire_id ],
			};
			// check for extra institutions
			if ( field[ f_mem_extra_institution_id ] ) {
			  var extra = field[ f_mem_extra_institution_id ].split(',');
			  for ( var ei = 0; ei < extra.length; ei++ ) {
				if ( parseInt( extra[ei] ) == 0 ) { continue; }
				member_info.institutions.push( parseInt(extra[ei]) );
			  }
			}
            mem_list.push( member_info );
        }

        var result = '';
		result += '<?xml version="1.0" encoding="UTF-8"?>' + "\n";
		result += '<!DOCTYPE collaborationauthorlist SYSTEM "author.dtd">' + "\n";
		result += '<collaborationauthorlist' + "\n";
		result += 'xmlns:foaf="http://xmlns.com/foaf/0.1/"' + "\n";
		result += 'xmlns:cal="http://www.slac.stanford.edu/spires/hepnames/authors_xml/">' + "\n";
		result += '<cal:creationDate>' + today + '</cal:creationDate>' + "\n";
		result += '<cal:publicationReference></cal:publicationReference>' + "\n";
  
		result += "<cal:collaborations>\n";
		result += "\t<cal:collaboration id=\"sphenix\">\n";
		result += "\t\t<foaf:name>SPHENIX</foaf:name>\n";
		result += "\t\t<cal:experimentNumber>BNL-RHIC-SPHENIX</cal:experimentNumber>\n";
		result += "\t</cal:collaboration>\n";
		result += "</cal:collaborations>\n";
		result += "<cal:organizations>\n";
		// TODO:
        inst_list = inst_list.sort( function( a, b ) {
            return a.full_name.toLowerCase() > b.full_name.toLowerCase();
        });
        for( var il = 0, illen = inst_list.length; il < illen; il++ ) {
		  result += "\t<foaf:Organization id=\"a" + inst_list[il].id + "\">\n";
		  if ( inst_list[il].short_name ) {
			result += "\t\t<foaf:name>" + inst_list[il].short_name + "</foaf:name>\n";
		  } else {
			result += "\t\t<foaf:name>" + inst_list[il].full_name + "</foaf:name>\n";
		  }
		  if ( inst_list[il].inspire_id ) {
      		result += "\t\t<cal:orgName source=\"INSPIRE\">" + inst_list[il].inspire_id + "</cal:orgName>\n";
		  }
		  result += "\t\t<cal:orgStatus collaborationid=\"sphenix\">Member</cal:orgStatus>\n";
		  result += "\t\t<cal:orgAddress>" + inst_list[il].address + "</cal:orgAddress>\n";
  		  result += "\t</foaf:Organization>\n";
        }
		result += "</cal:organizations>\n";
		result += '<cal:authors>' + "\n";
		// TODO:
        ...
        for( var ml = 0, mllen = mem_list.length; ml < mllen; ml++ ) {
		  result += '<foaf:Person>' + "\n";
		  result += "\t" + '<foaf:name>' + mem_list[ml].initials + ' ' + mem_list[ml].lastname + '</foaf:name>' + "\n";
		  result += "\t" + '<foaf:givenName>' + mem_list[ml].firstname + '</foaf:givenName>' + "\n";
		  result += "\t" + '<foaf:familyName>' + mem_list[ml].lastname + '</foaf:familyName>' + "\n";
		  result += "\t" + '<cal:authorNamePaper>' + mem_list[ml].papername + '</cal:authorNamePaper>' + "\n";
		  result += "\t" + '<cal:authorCollaboration collaborationid="sphenix"/>' + "\n";
		  result += "\t" + '<cal:authorAffiliations>' + "\n";
		  for ( var iin = 0; iin < mem_list[ml].institutions.length; iin++ ) {
			result += "\t" + "\t" + '<cal:authorAffiliation organizationid="a' + mem_list[ml].institutions[iin] + '"/>' + "\n";
		  }
		  result += "\t" + '</cal:authorAffiliations>' + "\n";
		  result += "\t" + '<cal:authorids>' + "\n";
		  if ( mem_list[ml].orcid ) {
			result += "\t\t" + '<cal:authorid source="ORCID">' + mem_list[ml].orcid + '</cal:authorid>' + "\n";
		  } else if ( mem_list[ml].inspire_id ) {
			result += "\t\t" + '<cal:authorid source="INSPIRE">INSPIRE-' 
			  + ( "00000000" + mem_list[ml].inspire_id ).slice(-8)
			  + '</cal:authorid>' + "\n";
		  }
		  result += "\t" + '</cal:authorids>' + "\n";
		  result += '</foaf:Person>' + "\n";
        ...
        var service = this;
        var label = 'Author List: INSPIRE/XML format';
        var tabid = service.addTab(label);
        $('#tabs').tabs('option', 'active', $('#'+tabid+'Selector').index());
		$('#'+tabid).html('<p><input type="button" onClick="downloadInnerHtml(\'authorlist-inspire-'+ now +'.xml\', \''+tabid+'-pre'
		  +'\', \'application/octet-stream\')" name="Download" value="DOWNLOAD" /></p><pre id="'+tabid+'-pre"></pre>');
        $('#'+tabid+'-pre').text( result );
    ...
    get_authorlist_aps: function() {
        ...
        for ( var i in this.members ) {
			...
			// FIXME: sphenix is not using is_author, all members are authors!
        ...
        $('#'+tabid).html('<p><input type="button" onClick="downloadInnerHtml(\'authorlist-aps-'+ now +'.tex\', \''+tabid+'-pre'+'\', \'application/octet-stream\')" name="Download" value="DOWNLOAD" /></p><pre id="'+tabid+'-pre"></pre>');
        $('#'+tabid+'-pre').text( result );
...
//        $('#'+tabid).html('<pre>Author List: IOP format' + "\n\n" + result + '</pre>');
        ...
        $('#'+tabid).html('<p><input type="button" onClick="downloadInnerHtml(\'authorlist-iop-'+ now +'.tex\', \''+tabid+'-pre'+'\', \'application/octet-stream\')" name="Download" value="DOWNLOAD" /></p><pre id="'+tabid+'-pre"></pre>');
        $('#'+tabid+'-pre').text( result );
	...
	display_statistics: function() {
		...
		var selects = '<p>I. sPHENIX Collaboration: <ul><li><b>'+num_mem+'</b> members</li><li><b>'+num_inst+'</b> institutions</li><li><b>'+num_cntr+'</b> countries (<b>'+num_reg+'</b> world regions)</li></ul> </p>';
    ...
    display_worldmap: function() {
        ...
        service.map.addControl(new L.Control.Layers( { 'Open Street Map': osm, 'Yandex': yandex, 'Yandex Hybrid': yandex_hybrid }, {}));
...
//        var google_sat = new L.Google('SATELLITE');
        var yandex = new L.Yandex();
        map.addControl(new L.Control.Layers( {'Open Street Map': osm, 'Yandex': yandex}, {}));
	...
	display_board: function() {
		var service = this;
		var label = 'sPHENIX Institutional Board';
		var tabid = service.addTab(label);
		$('#tabs').tabs('option', 'active', $('#'+tabid+'Selector').index());
    ...
    confirm_mass_emailr: function() {
      var service = this;
      var r = confirm("Do you want to send out notifications to IB Members, containing PhoneBook information?");
      if (r == true) {
        $.ajax({
